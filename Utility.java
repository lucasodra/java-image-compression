...
class VariableQuadtree {
    public static boolean isHomogeneous(int[][][] pixels, int xStart, int yStart, int width, int height, int varianceThreshold) {
        ...
        for (int x = xStart; x < xStart + width; x++) {
            for (int y = yStart - 1; y > yStart - height; y--) {
                ...
                varianceSum += redDiff * redDiff * greenDiff * greenDiff * blueDiff * blueDiff;
            }
        }
        int variance = varianceSum / pixelCount;
    ...
    public static QuadtreeNode buildQuadtree(int[][][] pixels, int xStart, int yStart, int width, int height, int sizeThreshold, int varianceThreshold) {
        QuadtreeNode node = new QuadtreeNode();

        if ((width * height <= sizeThreshold) || isHomogeneous(pixels, xStart, yStart, width, height, varianceThreshold)) {
        ...
        node.NW = buildQuadtree(pixels, xStart, yStart, midX, midY, sizeThreshold, varianceThreshold);
        node.NE = buildQuadtree(pixels, xStart + midX, yStart, midX, midY, sizeThreshold, varianceThreshold);
        node.SW = buildQuadtree(pixels, xStart, yStart - midY, midX, midY, sizeThreshold, varianceThreshold);
        node.SE = buildQuadtree(pixels, xStart + midX, yStart - midY, midX, midY, sizeThreshold, varianceThreshold);
...
public class Utility {
    ...
    public void Compress(int[][][] pixels, String outputFileName) throws IOException {
        QuadtreeNode root = VariableQuadtree.buildQuadtree(pixels, 0, pixels[0].length,pixels.length, pixels[0].length, 900, 10);
